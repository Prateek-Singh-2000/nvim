return {
  ...
  opts = {
    model_config = {
      ...
      -- This line uses an API constant, which you'll need to define or ensure is accessible.
      -- A common practice is to require the api module if it's exposed by the plugin.
      -- For simplicity, you might initially hardcode 'gemini-1.5-flash' or similar if 'api.MODELS.GEMINI_2_0_FLASH' isn't immediately available.
      -- For example: model_id = 'gemini-1.5-flash',
    ...
    hints = {
      ...
      get_prompt = function(node, bufnr)
        ...
        local prompt = " Instruction: Use 1 or 2 sentences to describe what the following {filetype} function does: ```{filetype} {code_block} ```"
